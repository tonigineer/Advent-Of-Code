...
fn solve2(input: &str, steps: u64) -> u64 {
    ...
    // Assumptions
    let size = g.grid.len();
    assert_eq!(g.grid.len(), g.grid[0].len());
    assert_eq!(sr, sc);
    assert_eq!(sr as usize, g.grid.len() / 2);
    ...
    let small_top_left  = fill(&g, (size - 1) as isize, (size - 1) as isize, (size / 2 - 1) as u64);
    ...
    let large_top_left  = fill(&g, (size - 1) as isize, (size - 1) as isize, (size * 3 / 2 - 1) as u64);
    ...
    return (
        num_odd_grids * odd_points as u64 +
        num_even_grids * even_points as u64 + 
        (corner_top + corner_right + corner_bottom + corner_left) as u64 +
        (grid_width + 1) * (small_top_right + small_top_left + small_bottom_right + small_bottom_left) as u64 + 
        grid_width * (large_top_right + large_top_left + large_bottom_right + large_bottom_left) as u64
    ) as u64;
}
...
fn fill(g: &common::Grid, sr: isize, sc: isize, steps: u64) -> u64 {
    ...
    while !q.is_empty() {
        ...
        for (dr, dc) in [(1isize, 0isize), (-1, 0), (0, 1), (0, -1)] {
            ...
            if nr < 0 || nr >= g.grid.len() as isize { continue }
            if nc < 0 || nc >= g.grid[0].len() as isize { continue }

            if g.grid[nr as usize][nc as usize] == '#' { continue }
