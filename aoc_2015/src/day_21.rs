...
impl Solution for Day21 {
    ...
    fn part1(&self, _: &str) -> Answer {
        return solve(false).into();
    }

    fn part2(&self, _: &str) -> Answer {
        return solve(true).into();
    }
}
...
impl Character {
    ...
    fn battle(&self, opponent: &Character) -> bool {
        ...
        loop {
            ...
            if hp_opp <= 0 { return true };
            if hp_self <= 0 { return false };
        }
    }
}
...
fn solve(part2: bool) -> u32 {
    ...
    if part2 {
        return iproduct!(&weapons, &armors, &rings, &rings)
            ...
            .map(
                |(w, a, r1, r2)| Character::new(&w, &a, &r1, &r2)
            )
            .filter(
                |h| !h.battle(&boss)
            )
