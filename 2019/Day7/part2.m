...
% Initialize
orig_opcode = load_opcode('opcode.txt');
permutations = perms([9,8,7,6,5]);
outputs = zeros(numel(permutations(:, 1)),1);

for permut=1:length(permutations)
    % Initialize new sequence
    amp = 1;
    amp_output = 0;
    sequence = permutations(permut, :);
    halt = false;
    initial_run = true;
    opcode = repmat(orig_opcode,1,5);
    pointer = ones(5,1);

    % Run intcode computer for the amps with the input (sequence and 
    % previous amp output)
    while ~halt
        for amp=1:numel(sequence)
            % Only at the initial iteration the sequence is part of the
            % input.
            if initial_run, inputs = [sequence(amp) amp_output];
            else, inputs = amp_output; end
            
            % For the loop, the state of the amp must remain as it were, when
            % its terminated. So pointer and opcode needs to be stored.
            % Some kind of class would be needed!!!!
            [halt, amp_output, opcode(:,amp), pointer(amp)] = ...
                intcode_computer(opcode(:,amp), inputs, pointer(amp));
        ...
        % Reminder that initial iteration is done
        initial_run = false;
    end
    outputs(permut) = amp_output;
    % Output sequence and store result in vector to determine maximum
%     fprintf(['Output to thruster: ' num2str(amp_output) '\n']);
end
    
fprintf(['Max. output to the thrusters: ' num2str(max(outputs)) '\n']);
