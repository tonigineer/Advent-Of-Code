...
for i=1:numel(positions(:,1))
    Y = [positions(i,2)-origin(2)];
    X = [positions(i,1)-origin(1)];
    positions(i,4) = X^2+Y^2;
    positions(i,3) = mod(atan2(Y, X)-pi/2, pi*2);
    if all(positions(i,1:2) == origin)
        positions(i,1:4) = inf;
    end
end

% Sort by heading (means clockwise)
[~, idx] = sortrows(positions,[3 4]);
positions = positions(idx,:);

% Loop over sorted list and vaporize asteroids clockwise (blocked ones can
% only be targeted during the next turn.
i = 0;
cangle = 0;
while true
    idx = find(positions(:,3)>=cangle,1,'first');
        i = i + 1;
    if i == 200
        fprintf('This is the planet: \n');
        positions(idx,:)
        (positions(idx,1)-1)*100+(positions(idx,2)-1)
        break
    end
    i
    cangle = mod(positions(idx,3)+1e-12,pi*2)
    positions(idx,:)
    positions(idx,:) = [];

    
    if numel(positions) == 4
        break
    end
