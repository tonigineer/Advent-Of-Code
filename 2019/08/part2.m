...
%     second layers, a black pixel in the third layer, and a white pixel in 
%     the fourth layer, the final image would have a black pixel at that 
%     position.
% 
...
img = img{1};
img_str = img{:};
for n=1:numel(img_str)
    img_arr(n) = str2double(img_str(n)); 
end
fclose(fid);
...
img_length = width*hight;
num_layer = numel(img_str)/(width*hight);

layer_decode = zeros(img_length, 1);

% Loop over string 
for pixel=1:img_length
    pixel_through_layer = img_arr(pixel:img_length:num_layer*img_length);
    pixel_through_layer(pixel_through_layer==2) = [];
    layer_decode(pixel) = pixel_through_layer(1);
end
raw = strrep(num2str(layer_decode'),' ', '');
fprintf(strrep(strrep(raw,'0', ' '), '1', 'X'));
fprintf('\n\n');



raw_mat = vec2mat(layer_decode,width,hight);

for i=1:size(raw_mat)
    raw = strrep(num2str(raw_mat(i, :)),' ', '');
    fprintf(['Result: ' strrep(strrep(raw,'0', ' '), '1', 'X') '\n']);
end
