...
% password. The Elves had written the password on a sticky note, but someone threw it out.
% 
...
for possible_password=range(1):range(2)
    ...
    % check adjacent_digits criterion
    adjacent_digits = false;
    for digit=1:5
        if password_digits(digit) == password_digits(digit+1)
            if sum(password_digits == password_digits(digit)) == 2
    ...
    % skip, already one criterion failed
    if ~adjacent_digits
        continue
    end

    % check never_decrease criterion
    never_decrease = true;
    for digit=1:5
        if password_digits(digit) > password_digits(digit+1)
          adjacent_digits = false;
